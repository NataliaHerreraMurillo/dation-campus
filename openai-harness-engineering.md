...
So instead of treating AGENTS.md as the encyclopedia, we treat it as the table of contents. The repository’s knowledge base lives in a structured `docs/` directory treated as the system of record. A short AGENTS.md (roughly 100 lines) is injected into context and serves primarily as a map.
...
Because the repository is entirely agent-generated, it’s optimized first for Codex’s legibility. Our human engineers’ goal was making it possible for an agent to reason about the full business domain directly from the repository itself.

From the agent’s point of view, anything it can’t access in-context while running effectively doesn’t exist. We learned that we needed to push more and more context into the repo over time. If a Slack discussion aligned the team on an architectural pattern, it must be documented in the repo to be "legible" to the agent.

---

### Enforcing architecture and taste

Documentation alone doesn’t keep a fully agent-generated codebase coherent. We built the application around a rigid architectural model. Each business domain is divided into a fixed set of layers (Types → Config → Repo → Service → Runtime → UI), with strictly validated dependency directions.

These constraints are enforced mechanically via custom linters (Codex-generated) and structural tests. In a human-first workflow, these rules might feel pedantic. With agents, they become multipliers: once encoded, they apply everywhere at once.
...
As Codex’s throughput increased, many conventional engineering norms became counterproductive. The repository operates with minimal blocking merge gates. Pull requests are short-lived. In a system where agent throughput far exceeds human attention, corrections are cheap, and waiting is expensive.
...
Humans always remain in the loop, but work at a different layer of abstraction. We prioritize work, translate user feedback into acceptance criteria, and validate outcomes.

---

### Increasing levels of autonomy

Recently, the repository crossed a threshold where Codex can end-to-end drive a new feature. Given a single prompt, the agent can:
...
8. Respond to feedback and remediate build failures
9. Merge the change
...
Initially, humans addressed "AI slop" manually, spending 20% of the week cleaning up. To scale this, we encoded “golden principles” directly into the repo and built a recurring cleanup process. Background Codex tasks scan for deviations, update quality grades, and open targeted refactoring pull requests. This functions like garbage collection for technical debt.

---

### What we’re still learning

While this strategy has worked well for internal launch at OpenAI, we are still learning how architectural coherence evolves over years in a fully agent-generated system. Building software still demands discipline, but the discipline now shows up more in the scaffolding rather than the code.

Our most difficult challenges now center on designing environments, feedback loops, and control systems that help agents build and maintain complex, reliable software at scale.
